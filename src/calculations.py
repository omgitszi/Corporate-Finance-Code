...
def compute_annualized_covariance(
    ...
    # Default output folder is <repo>/src/output (one level up from package)
    ...
    corr_monthly.to_csv(str(corr_file))
...
if __name__ == "__main__":
    ...
    # set output directory under src/output and ensure it exists
    output_dir = Path(__file__).resolve().parent / "output"
    output_dir.mkdir(parents=True, exist_ok=True)

    output_file = output_dir / "summary.csv"
    pivoted = asset_summary.set_index("symbol").T
    pivoted.to_csv(str(output_file))

    fig = build_assets_figure(asset_summary)
    out_path = output_dir / "assets_figure.png"
    ...
    cov = compute_annualized_covariance("monthly_returns.csv", symbols=symbols, output_dir=output_dir)
    print("Saved covariance matrix to:", str(output_dir / "covariance_matrix.csv"))